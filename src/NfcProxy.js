...
class NfcProxy {
  ...
  naiveFormatNdefNfcA = async () => {
    ...
    try {
      ...
      const naiveFormatPayload = [0xa2, 0x04, 0x03, 0x00, 0xfe, 0x00];

      if (Platform.OS === 'ios') {
        resp = await NfcManager.sendMifareCommandIOS(naiveFormatPayload);
      } else {
        resp = await NfcManager.transceive(naiveFormatPayload);
      }
    } catch (ex) {
      console.warn(ex);
      err = ex;
